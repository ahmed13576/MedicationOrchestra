...
The audit command in §0 now exits **0**: `39/39` checks hold, and the test-suite
is `121 passed`. The table below is the honest ledger of what changed and what
still has not.
...
| F-11 | No tests, no CI, no evaluation harness | **Fixed** | 121 tests (in-memory Firestore, no credentials), 39 executed invariant checks, and `.github/workflows/ci.yml` running ruff, pip-audit, the invariant audit, pytest, a knowledge-base integrity check and a container build. Remaining: no evaluation corpus for extraction accuracy. |
