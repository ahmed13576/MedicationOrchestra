...
| F-15 | Repository hygiene and documentation drift | **Fixed** | hardcoded project ids removed from both deployment scripts and test-enforced repository-wide (the client's generated Firebase config is the only exception, and it is listed in the test), a root `firebase.json`/`.firebaserc` was added so the documented `firebase deploy --only firestore:rules` actually works, `agents/.adk/session.db` untracked, generated corpus removed and gitignored, provenance documented in `docs/KNOWLEDGE_SOURCES.md`. The `gumloop/` sprint skills and `adapters/` scaffolding were deleted — they instructed the next contributor to rebuild the rejected design, including a hand-written brand→generic map matched with `startswith`, the substring bug F-03 is about. `docs/HISTORY.md` replaces them. |
